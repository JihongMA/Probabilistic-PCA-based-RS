...
object PPCAExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ratings = fromCSV(args(5).toString, sparkSession)
    ...
    val entries = ratings.df.rdd.map(row => MatrixEntry(row(0).asInstanceOf[Double].toLong,
      row(1).asInstanceOf[Double].toLong, row(2).asInstanceOf[Double]))
//    val mat = new CoordinateMatrix(entries, nrow.toLong, ncol.toLong)

    val rows = entries.map(entry => (entry.i, (entry.j.toInt, entry.value)))
      .groupByKey()
      .map { case (i, vectorEntries) =>
        Vectors.sparse(ncol, vectorEntries.toSeq)
      }

    val rowMat = new RowMatrix(rows, nrow.toLong, ncol)
...
//    val ratingsRow = ratingsIdx.toRowMatrix()
     ...
     /**
      * args(0) number of principle components (d)
      */

    val ppca = PPCA(args(0).toInt, args(1).toInt, args(2).toFloat,
      args(3).toDouble, args(4).toDouble)

    val C = ppca.computePCA(rowMat, ym)
